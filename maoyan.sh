...
##		20140902		0.6		增加投资者资金对帐功能
##		20140915		0.7		修改报盘测试方法
###
###
###
#########################################################################
...
function _InvestorSettlementCheck(){
	echo -ne "\n\n${green} 单个投资者资金对帐，请输入投资者资金帐号: ${end}"
	read investor_no
...
EOF`
	...
	if [ "${sysqy}" == "${csvqy}" ];then
		echo -e "\n${green} 投资者权益一致,投资者【${investor_no}】的权益是:  ${end}${yellow}${sysqy}${end}"
	else
		echo -e "\n${red} 投资都权益不一致,数据库中投资者【${investor_no}】的权益是:  ${end}${yellow}${sysqy}${end}${red} ,CSV文件中投资者【${investor_no}】的权益是: ${end}${yellow}${csvqy}${end}"
	fi
	if [ "${syskyzj}" == "${csvkyzj}" ];then
		echo -e "\n${green} 投资者可用资金一致,投资者【${investor_no}】的可用资金是:  ${end}${yellow}${syskyzj}${end}"
	else
		echo -e "\n${red} 投资者的可用资金不一致,数据库中投资者【${investor_no}】的可用资金是:  ${end}${yellow}${syskyzj}${end}${red} , CSV文件中投资者【${investor_no}】的可用资金是: ${end}${yellow}${csvkyzj}${end}"
	fi
	if [ "${sysbzj}" == "${csvbzj}" ];then
		echo -e "\n${green} 投资者保证金一致,投资者【${investor_no}】的总保证金是:  ${end}${yellow}${sysbzj}${end}"
	else
		echo -e "\n${red} 投资者的总保证金不一致,数据库中投资者【${investor_no}】的保证金是:  ${end}${yellow}${syszj}${end}${red} , CSV文件中投资者【${investor_no}】的保证金是: ${end}${yellow}${csvzj}${end}"
	fi
	if [ "${sysmai}" == "${csvmai}" ];then
		echo -e "\n${green} 投资者买持仓一致,投资者【${investor_no}】的总买持仓:  ${end}${yellow}${sysmai}${end}"
	else
		echo -e "\n${red} 投资者的买持仓不一致,数据库中投资者【${investor_no}】的总买持仓是:  ${end}${yellow}${sysmai}${end}${red} ,CSV文件中投资者【${investor_no}】的总买持仓是: ${end}${yellow}${csvmai}${end}"
	fi
	if [ "${sysmai4}" == "${csvmai4}" ];then
		echo -e "\n${green} 投资者卖持仓一致,投资者【${investor_no}】的总卖持仓:  ${end}${yellow}${sysmai4}${end}"
	else
		echo -e "\n${red} 投资者的卖持仓不一致,数据库中投资者【${investor_no}】的总卖持仓是:  ${end}${yellow}${sysmai4}${end}${red} ,CSV文件中投资者【${investor_no}】的总卖持仓是: ${end}${yellow}${csvmai4}${end}"
	fi
	if [ "${sysqy}" == "${csvqy}" ] && [ "${syskyzj}" == "${csvkyzj}" ] && [ "${sysbzj}" == "${csvbzj}" ] && [ "${sysmai}" == "${csvmai}" ] && [ "${sysmai4}" == "${csvmai4}" ]; then
		echo -e "\n${yellow} 投资者【${investor_no}】资金对帐完成...${end}"
		echo -en "\n${green} 是否继续进行其他投资者资金对帐【y/n】： ${end}"
		read nchs
...
function menu(){
	...
	test $DEBUG -gt 0 && echo -e "\t\t6.投资者资金对帐\t"
	test $DEBUG -gt 0 && echo -e "\t\t7.报盘启动测试\t"
    echo -e "\t\t0.退出\t" 
	echo -ne "\t\t请选择操作: ${end}"
}
if [ $* > 0 ];then
